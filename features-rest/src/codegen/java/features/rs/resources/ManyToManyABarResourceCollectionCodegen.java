  ...
  public PagedCollectionBinding get(final @Context Repository repo, final @QueryParam("startIndex") Integer startIndexParam, final @QueryParam("maxResults") Integer maxResultsParam, final @QueryParam("name") String name) {
    return UoW.read(repo, new BlockWithReturn<PagedCollectionBinding>() {
      public PagedCollectionBinding go() {
        ...
        ManyToManyABarAlias mtmab0 = new ManyToManyABarAlias();
        Select<ManyToManyABar> q = Select.from(mtmab0);
        if(name != null) {
          q.where(mtmab0.name.eq(name));
        }
        q.orderBy(mtmab0.id.asc());
        q.offset(startIndex);
        q.limit(maxResults );
        List<ManyToManyABar> list = q.list();
        PagedCollectionBinding result = new PagedCollectionBinding();
        result.setLinksFromDomainObjects(list);
        if (startIndex > 0) {
          result.setPrevious(new CollectionLinkBinding("previous", new UriBuilderImpl().path(ManyToManyABarResourceCollectionCodegen.class) .queryParam("startIndex", Math.max(0, startIndex - maxResults)).queryParam("maxResults", Math.min(startIndex, maxResults)).build().toString()));
        }
        if (!list.isEmpty() && list.size() == maxResults) {
          result.setNext(new CollectionLinkBinding("next", new UriBuilderImpl().path(ManyToManyABarResourceCollectionCodegen.class).queryParam("startIndex", startIndex + maxResults).queryParam("maxResults", maxResults).build().toString()));
        }
        return result;
      }
    });
  ...
  public Long post(final @Context Repository repo, final ManyToManyABarBinding manyToManyABar) {
    return UoW.go(repo, null, new BlockWithReturn<ManyToManyABar>() {
      public ManyToManyABar go() {
        ...
        toDomain(manyToManyABar, domainObject);
        return domainObject;
      }
    }).getId();
  }
