...
public class ParentDToChildCResourceCollectionCodegen {
  ...
  public PagedCollectionBinding get(final @Context Repository repo, final @QueryParam("startIndex") Integer startIndexParam, final @QueryParam("maxResults") Integer maxResultsParam, final @QueryParam("parentDChildC") Long parentDChildC, final @QueryParam("parentD") Long parentD) {
    return UoW.read(repo, new BlockWithReturn<PagedCollectionBinding>() {
      public PagedCollectionBinding go() {
        ...
        q.orderBy(pdtcc0.id.asc());
        q.offset(startIndex);
        q.limit(maxResults );
        List<ParentDToChildC> list = q.list();
        PagedCollectionBinding result = new PagedCollectionBinding();
        result.setLinksFromDomainObjects(list);
        if (startIndex > 0) {
          result.setPrevious(new CollectionLinkBinding("previous", new UriBuilderImpl().path(ParentDToChildCResourceCollectionCodegen.class) .queryParam("startIndex", Math.max(0, startIndex - maxResults)).queryParam("maxResults", Math.min(startIndex, maxResults)).build().toString()));
        }
        if (!list.isEmpty() && list.size() == maxResults) {
          result.setNext(new CollectionLinkBinding("next", new UriBuilderImpl().path(ParentDToChildCResourceCollectionCodegen.class).queryParam("startIndex", startIndex + maxResults).queryParam("maxResults", maxResults).build().toString()));
        }
        return result;
      }
    });
  ...
  public Long post(final @Context Repository repo, final ParentDToChildCBinding parentDToChildC) {
    return UoW.go(repo, null, new BlockWithReturn<ParentDToChildC>() {
      public ParentDToChildC go() {
        ...
        toDomain(parentDToChildC, domainObject);
        return domainObject;
      }
    }).getId();
  }
